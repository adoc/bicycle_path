...
class PlayerStrategy(object):
    def __init__(self,strattype='basic'):
        ...
        hardfile = open('data/blackjack/strategy/%s_hard.csv'%(strattype),'rb')
        softfile = open('data/blackjack/strategy/%s_soft.csv'%(strattype),'rb')
        pairsfile = open('data/blackjack/strategy/%s_pairs.csv'%(strattype),'rb')
